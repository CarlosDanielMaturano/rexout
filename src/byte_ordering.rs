...
mod test {
    ...
    fn double_byte() {
        ...
        assert_eq!(vec![0x6465], order_bytes(bytes, true).unwrap())
    }
    ...
    fn big_endian_hex_alphabet_representation() {
        ...
        assert_eq!(expected, order_bytes(alphabet, false).unwrap());
    }
    ...
    fn odd_byte_ordering() {
        ...
        assert_eq!(expected, order_bytes(bytes, true).unwrap());
    }
}
